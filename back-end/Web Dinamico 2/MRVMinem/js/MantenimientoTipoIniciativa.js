...
function fn_CargaDatos() {
    ...
    $.ajax({
        url: baseUrl + "Mantenimiento/ListaTipoIniciativa",
        type: 'POST',
        datatype: 'json',
        data: Item,
            ...
            if (data != null && data != "") {
                if (data.length > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        var tr = '<tr>';
                        tr = tr + '<th class="text-center" data-encabezado="Número" scope="col">' + data[i]["ID_TIPO_INICIATIVA"] + '</th>';
                        tr = tr + '<td data-encabezado="Descripcion">' + data[i]["TIPO_INICIATIVA"] + '</td>';
                        ...
                        tr = tr + '             <a class="dropdown-item" href="#" onclick="fn_cargarDatosMantenimiento(' + data[i]["ID_TIPO_INICIATIVA"] + ');" data-toggle="modal" data-target="#modal-edicion">';
                        tr = tr + '                 <i class="fas fa-edit"></i>&nbsp;Editar';
                        tr = tr + '             </a><a class="dropdown-item" href="#" onclick="fn_Seleccionaregistro(' + data[i]["ID_TIPO_INICIATIVA"] + ');" data-toggle="modal" data-target="#modal-confirmacion">';
...
function fn_seleccionarMantenimiento(id) {
    var Item = {
        ID_TIPO_INICIATIVA: id
    };
    $.ajax({
        url: baseUrl + "Mantenimiento/BuscarTipoIniciativa",
        type: 'POST',
        datatype: 'json',
        data: Item,
        success: function (data) {
            if (data != null && data != "") {
                $("#userMantenimiento").data("value", data["ID_TIPO_INICIATIVA"]);
                $("#txt-descripcion").val(data["TIPO_INICIATIVA"]);
            }
        }
    });
}
...
function fn_editarMantenimiento() {
    ...
    var url = baseUrl + "Mantenimiento/RegistrarTipoIniciativa";
    var item = {
        ID_TIPO_INICIATIVA: $("#userMantenimiento").data("value"),
        TIPO_INICIATIVA: $("#txt-descripcion").val(),
...
function fn_eliminarMantenimiento() {
    var url = baseUrl + "Mantenimiento/EliminarTipoIniciativa";
    var item = {
        ID_TIPO_INICIATIVA: $("#userMantenimiento").data("value")
    };
    var mensaje = "";
...
function exportarMantenimiento() {
    ...
    var url = baseUrl + 'Mantenimiento/ExportarMantenimientoTipoIniciativa';
