...
function fn_CargaDatos() {
    ...
    $.ajax({
        url: baseUrl + "Mantenimiento/ListaNama",
        type: 'POST',
        datatype: 'json',
        data: Item,
            ...
            if (data != null && data != "") {
                if (data.length > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        var tr = '<tr>';
                        tr = tr + '<th class="text-center" data-encabezado="Número" scope="col">' + data[i]["ID_NAMA"] + '</th>';
                        tr = tr + '<td data-encabezado="Descripcion">' + data[i]["DESCRIPCION_NAMA"] + '</td>';
                        ...
                        tr = tr + '             <a class="dropdown-item" href="#" onclick="fn_cargarDatosMantenimiento(' + data[i]["ID_NAMA"] + ');" data-toggle="modal" data-target="#modal-edicion">';
                        tr = tr + '                 <i class="fas fa-edit"></i>&nbsp;Editar';
                        tr = tr + '             </a><a class="dropdown-item" href="#" onclick="fn_Seleccionaregistro(' + data[i]["ID_NAMA"] + ');" data-toggle="modal" data-target="#modal-confirmacion">';
...
function fn_seleccionarMantenimiento(id) {
    ...
    $.ajax({
        url: baseUrl + "Mantenimiento/BuscarNama",
        type: 'POST',
        datatype: 'json',
        data: Item,
        success: function (data) {
            if (data != null && data != "") {
                $("#userMantenimiento").data("value", data["ID_NAMA"]);
                $("#txt-descripcion").val(data["DESCRIPCION_NAMA"]);
            }
        }
    });
}
...
function fn_editarMantenimiento() {
    ...
    var url = baseUrl + "Mantenimiento/RegistrarNama";
    var item = {
        ID_NAMA: $("#userMantenimiento").data("value"),
        DESCRIPCION_NAMA: $("#txt-descripcion").val(),
        FLG_ESTADO: $("#validarUsuario").data("guardar")
    };
    var mensaje = "";
...
function fn_validarCampo() {
    ...
    if ($("#txt-descripcion").val().trim() === "") {
        arr.push("Debe ingresar el nombre de la Nama");
    }
...
function fn_eliminarMantenimiento() {
    var url = baseUrl + "Mantenimiento/EliminarNama";
    var item = {
        ID_NAMA: $("#userMantenimiento").data("value")
    };
    var mensaje = "";
...
function exportarMantenimiento() {
    ...
    var url = baseUrl + 'Mantenimiento/ExportarMantenimientoNama';
