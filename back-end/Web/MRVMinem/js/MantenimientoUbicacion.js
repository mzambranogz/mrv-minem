...
function fn_CargaDatos() {
    ...
    $.ajax({
        url: baseUrl + "Mantenimiento/ListaUbicacion",
        type: 'POST',
        datatype: 'json',
        data: Item,
            ...
            if (data != null && data != "") {
                if (data.length > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        var tr = '<tr>';
                        tr = tr + '<th class="text-center" data-encabezado="Número" scope="col">' + data[i]["ID_UBICACION"] + '</th>';
                        ...
                        tr = tr + '             <a class="dropdown-item" href="#" onclick="fn_cargarDatosMantenimiento(' + data[i]["ID_UBICACION"] + ');" data-toggle="modal" data-target="#modal-edicion">';
                        tr = tr + '                 <i class="fas fa-edit"></i>&nbsp;Editar';
                        tr = tr + '             </a><a class="dropdown-item" href="#" onclick="fn_Seleccionaregistro(' + data[i]["ID_UBICACION"] + ');" data-toggle="modal" data-target="#modal-confirmacion">';
...
function fn_seleccionarMantenimiento(id) {
    ...
    $.ajax({
        url: baseUrl + "Mantenimiento/BuscarUbicacion",
        type: 'POST',
        datatype: 'json',
        data: Item,
        success: function (data) {
            if (data != null && data != "") {
                $("#userMantenimiento").data("value", data["ID_UBICACION"]);
...
function fn_editarMantenimiento() {
    ...
    var url = baseUrl + "Mantenimiento/RegistrarUbicacion";
    var item = {
        ID_UBICACION: $("#userMantenimiento").data("value"),
...
function fn_validarCampo() {
    ...
    if ($("#txt-descripcion").val().trim() === "") {
        arr.push("Debe ingresar el nombre de la Ubicación");
    }
