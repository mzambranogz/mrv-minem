...
function fn_CargaDatos() {
    ...
    $.ajax({
        url: baseUrl + "Mantenimiento/ListaSectores",
        type: 'POST',
        datatype: 'json',
        data: Item,
            ...
            if (data != null && data != "") {
                if (data.length > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        var tr = '<tr>';
                        tr = tr + '<th class="text-center" data-encabezado="Número" scope="col">' + data[i]["ID_SECTOR_INST"] + '</th>';
...
function fn_seleccionarMantenimiento(id) {
    ...
    $.ajax({
        url: baseUrl + "Mantenimiento/Buscarsector",
        type: 'POST',
        datatype: 'json',
        data: Item,
        success: function (data) {
            if (data != null && data != "") {
                $("#userMantenimiento").data("value", data["ID_SECTOR_INST"]);
...
function fn_editarMantenimiento() {
    ...
    var url = baseUrl + "Mantenimiento/RegistrarSector";
    var item = {
        ID_SECTOR_INST: $("#userMantenimiento").data("value"),
...
function fn_validarCampo() {
    ...
    if ($("#txt-descripcion").val().trim() === "") {
        arr.push("Debe ingresar el nombre del sector");
    }
