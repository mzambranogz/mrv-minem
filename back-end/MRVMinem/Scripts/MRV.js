...
MRV = {
    ...
    MultiplicaNumeros: function (ListaNumeros, decimal) {
        ...
        $.each(ListaNumeros, function (k, Numero) {
            ...
            if (posCaracterDecimal > 0) {
                ...
                numeroEntero = parseInt(parteEntera) * parseInt("1" + MRV.PadLeft("", parteDecima.toString().length, "0")) + parseInt(parteDecima);
        ...
        if (Producto.toString().length > cantDecimal) {
            ...
            resultado = "0." + MRV.PadLeft(Producto.toString(), dif, "0");
        }
        if (decimal != null) {
    ...
    GetTimeToDate: function (date) {
        ...
        if (date != null && date != "") {
            if (date.indexOf("/Date(") >= 0) {
                ...
                var fecharecepcion = MRV.getDate(fecha) + '/' + MRV.getMonth(fecha) + '/' + fecha.getFullYear();
    ...
    Grilla: function (grilla, pager, identificador, height, width, caption, urlListar, id, colsNames, colsModel, sortName, opciones) {
                ...
                gridComplete: function () {
                    if ($('#' + grilla).getGridParam('records') == 0) {
                        if (opciones.noregistro == true) {
                            MRV.ShowAlert("dialog-alert", "Sin Registro");
                        }
                    }
                ...
                onClickButton: function () {
                    if (rowKey != null) {
                        if (opciones.btnEliminar == null) {
                            MRV.confirm(opciones.Mensaje, function () { Eliminar(); }, '', opciones.dialogDelete, opciones.Lenguaje);
                        }
                        else {
                            if (opciones.btnEliminar.Mensaje != null && opciones.btnEliminar.Mensaje != '') {
                                MRV.confirm(opciones.btnEliminar.Mensaje, function () { opciones.btnEliminar.Function(rowKey); }, '', opciones.btnEliminar.titleMensaje, opciones.Lenguaje);
                            }
                            else {
                        ...
                        if (opciones.Alert == null) {
                            MRV.Alert(opciones.dialogAlert, opciones.Mensaje, '', opciones.Lenguaje);
                        }
                        else {
                            MRV.Alert(opciones.Alert.titleMensaje, opciones.Alert.Mensaje, '', opciones.Lenguaje);
                        }
                    }
                }
            });
        ...
        var funccion = 'MRV.LayoutBarra("' + pager.toString() + '")';
    ...
    LayoutBarra: function (pager) {
        ...
        if (pager != null && pager != '') {
            ...
            //  width left
            if (wl < wlc) {
                jQuery("#" + pager + "_left").attr('style', 'width:' + (wlc) + 'px');
            }
    ...
    LoadDropDownListItems: function (name, url, parameters, selected) {
        ...
        $('#' + name + ' > option[value="' + selected + '"]').attr('selected', 'selected');
        //$('#' + name).val(selected);
    },
    ...
    BloquearControlesTodos: function (contenedor) {
        ...
        jQuery('#' + contenedor).find('input:checkbox').attr('disabled', true);
    ...
    CargarComboGrilla: function (url, parameters) {
        ...
        var cadena = "";
        var respuesta = MRV.Ajax(url, parameters, false);
        if (respuesta != null) {
    ...
    InicializarMsg: function () {
        $(".msgModel").show();
        MRV.msgHide();
        MRV.ShowElement(".msgModel .loading");
    },
    ...
    msgConfirm: function (rpt, Message, funcionout) {
        ...
        if (rpt) {
            $(".msgModel").slideUp(200, function () {
                ...
                MRV.ShowElement(".msgModel .mexito");
            ...
            $(".msgModel").slideUp(200, function () {
                ...
                MRV.ShowElement(".msgModel .merror");
            });
        }
        $(".msgModel").show();
    ...
    ExportarExcel: function (obj, url, content, barra) {
        ...
        var varbarra = jQuery('#pg_' + barra).html();
        var varseleccion = jQuery('.ui-pg-selbox').html();
        var inputtext = jQuery('.ui-pg-input').val();
        var texto = jQuery('.ui-paging-info').html();
        jQuery('#pg_' + barra).html('');
        if (varseleccion != undefined || varseleccion != null) jQuery('.ui-pg-selbox').html('');
        if (inputtext != undefined || inputtext != null) jQuery('.ui-pg-input').val('');
        if (texto != undefined || texto != null) jQuery('.ui-paging-info').html('');
        var clonado = jQuery(content).parent().eq(0).clone();
        jQuery('#hdnExportarExcel').val($('<div />').text(clonado.html()).html());
        jQuery('#frmExcel').submit();
        jQuery('#frmExcel').remove();
        jQuery('#pg_' + barra).html(varbarra);
        if (varseleccion != undefined || varseleccion != null) jQuery('.ui-pg-selbox').html(varseleccion);
        if (inputtext != undefined || inputtext != null) jQuery('.ui-pg-input').val(inputtext);
        if (texto != undefined || texto != null) jQuery('.ui-paging-info').html(texto);
...
function MostrarHojaTramite(pUrlHojaTramite) {
    window.open(pUrlHojaTramite, '_blank');
}

function InvocarPagina(url, tipo) {
...
function InvocarVista(url, tipo, estdoc, idOficina) {
...
function InvocarVistaReporte(url, idOficina) {
    var item =
   {
