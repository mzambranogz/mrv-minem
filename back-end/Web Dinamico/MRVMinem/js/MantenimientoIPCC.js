...
function fn_CargaDatos() {
    ...
    $.ajax({
        url: baseUrl + "Mantenimiento/ListaIPCC",
        type: 'POST',
        datatype: 'json',
        data: Item,
            ...
            if (data != null && data != "") {
                if (data.length > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        var tr = '<tr>';
                        tr = tr + '<th class="text-center" data-encabezado="Número" scope="col">' + data[i]["ID_IPCC"] + '</th>';
                        tr = tr + '<td data-encabezado="Descripcion">' + data[i]["IPCC"] + '</td>';
                        ...
                        tr = tr + '             <a class="dropdown-item" href="#" onclick="fn_cargarDatosMantenimiento(' + data[i]["ID_IPCC"] + ');" data-toggle="modal" data-target="#modal-edicion">';
                        tr = tr + '                 <i class="fas fa-edit"></i>&nbsp;Editar';
                        tr = tr + '             </a><a class="dropdown-item" href="#" onclick="fn_Seleccionaregistro(' + data[i]["ID_IPCC"] + ');" data-toggle="modal" data-target="#modal-confirmacion">';
...
function fn_seleccionarMantenimiento(id) {
    ...
    $.ajax({
        url: baseUrl + "Mantenimiento/BuscarIPCC",
        type: 'POST',
        datatype: 'json',
        data: Item,
        success: function (data) {
            if (data != null && data != "") {
                $("#userMantenimiento").data("value", data["ID_IPCC"]);
                $("#txt-descripcion").val(data["IPCC"]);
            }
        }
    });
}
...
function fn_editarMantenimiento() {
    ...
    var url = baseUrl + "Mantenimiento/RegistrarIPCC";
    var item = {
        ID_IPCC: $("#userMantenimiento").data("value"),
        IPCC: $("#txt-descripcion").val(),
...
function fn_validarCampo() {
    ...
    if ($("#txt-descripcion").val().trim() === "") {
        arr.push("Debe ingresar el nombre de la IPCC");
    }
...
function fn_eliminarMantenimiento() {
    var url = baseUrl + "Mantenimiento/EliminarIPCC";
    var item = {
        ID_IPCC: $("#userMantenimiento").data("value")
    };
    var mensaje = "";
...
function exportarMantenimiento() {
    ...
    var url = baseUrl + 'Mantenimiento/ExportarMantenimientoIPCC';
