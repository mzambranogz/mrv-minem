...
function fn_CargaDatos() {
    ...
    $.ajax({
        url: baseUrl + "Mantenimiento/ListaRoles",
        type: 'POST',
        datatype: 'json',
        data: Item,
            ...
            if (data != null && data != "") {
                if (data.length > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        var tr = '<tr>';
                        tr = tr + '<th class="text-center" data-encabezado="Número" scope="col">' + data[i]["ID_ROL"] + '</th>';
                        tr = tr + '<td data-encabezado="Descripcion">' + data[i]["DESCRIPCION_ROL"] + '</td>';
                        ...
                        tr = tr + '             <a class="dropdown-item" href="#" onclick="fn_cargarDatosMantenimiento(' + data[i]["ID_ROL"] + ');" data-toggle="modal" data-target="#modal-edicion">';
                        tr = tr + '                 <i class="fas fa-edit"></i>&nbsp;Editar';
                        tr = tr + '             </a>';
                        //tr = tr + '             <a class="dropdown-item" href="#" onclick="fn_Seleccionaregistro(' + data[i]["ID_ROL"] + ');" data-toggle="modal" data-target="#modal-confirmacion">';
...
function fn_seleccionarMantenimiento(id) {
    ...
    $.ajax({
        url: baseUrl + "Mantenimiento/BuscarRol",
        type: 'POST',
        datatype: 'json',
        data: Item,
        success: function (data) {
            if (data != null && data != "") {
                $("#userMantenimiento").data("value", data["ID_ROL"]);
                $("#txt-descripcion").val(data["DESCRIPCION_ROL"]);
            }
        }
    });
}
...
function fn_editarMantenimiento() {
    ...
    var url = baseUrl + "Mantenimiento/RegistrarRol";
    var item = {
        ID_ROL: $("#userMantenimiento").data("value"),
        DESCRIPCION_ROL: $("#txt-descripcion").val(),
...
function fn_validarCampo() {
    ...
    if ($("#txt-descripcion").val().trim() === "") {
        arr.push("Debe ingresar el nombre del rol");
    }
...
function fn_eliminarMantenimiento() {
    var url = baseUrl + "Mantenimiento/EliminarRol";
    var item = {
        ID_ROL: $("#userMantenimiento").data("value")
    };
    var mensaje = "";
...
function exportarMantenimiento() {
    ...
    var url = baseUrl + 'Mantenimiento/ExportarMantenimientoRoles';
