...
function fn_CargaDatos() {
    ...
    $.ajax({
        url: baseUrl + "Mantenimiento/ListaPlazo",
        type: 'POST',
        datatype: 'json',
        data: Item,
            ...
            if (data != null && data != "") {
                if (data.length > 0) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        var tr = '<tr>';
                        tr = tr + '<th class="text-center" data-encabezado="Número" scope="col">' + data[i]["ID_PLAZO_ETAPA_ESTADO"] + '</th>';
                        tr = tr + '<td data-encabezado="Descripcion">' + data[i]["ETAPA"] + '</td>';
                        tr = tr + '<td data-encabezado="Descripcion" class="text-center">' + data[i]["ESTADO"] + '</td>';
                        tr = tr + '<td data-encabezado="Descripcion" class="text-center">' + data[i]["DESCRIPCION_ROL"] + '</td>';
                        tr = tr + '<td data-encabezado="Descripcion">' + data[i]["DESCRIPCION"] + '</td>';
                        tr = tr + '<td data-encabezado="Descripcion" class="text-center">' + data[i]["PLAZO"] + '</td>';
                        ...
                        tr = tr + '             <a class="dropdown-item" href="#" onclick="fn_cargarDatosMantenimiento(' + data[i]["ID_PLAZO_ETAPA_ESTADO"] + ');" data-toggle="modal" data-target="#modal-edicion">';
                        tr = tr + '                 <i class="fas fa-edit"></i>&nbsp;Editar</a>';
                        //tr = tr + '             <a class="dropdown-item" href="#" onclick="fn_Seleccionaregistro(' + data[i]["ID_PLAZO_ETAPA_ESTADO"] + ');" data-toggle="modal" data-target="#modal-confirmacion">';
                        //tr = tr + '                 <i class="fas fa-trash"></i>&nbsp;Eliminar</a>';
                        tr = tr + '         </div>';
                        tr = tr + "     </div></td>";
                        tr = tr + '</tr>';
...
function fn_seleccionarMantenimiento(id) {
    var Item = {
        ID_PLAZO_ETAPA_ESTADO: id
    };
    $.ajax({
        url: baseUrl + "Mantenimiento/BuscarPlazo",
        type: 'POST',
        datatype: 'json',
        data: Item,
        success: function (data) {
            if (data != null && data != "") {
                $("#userMantenimiento").data("value", data["ID_PLAZO_ETAPA_ESTADO"]);
                $("#txt-etapa").val(data["ETAPA"]);
                $("#txt-estado").val(data["ESTADO"]);
                $("#txt-responsable").val(data["DESCRIPCION_ROL"]);
                $("#txa-descripcion").val(data["DESCRIPCION"]);
                $("#txt-plazo").val(data["PLAZO"]);
            }
        }
    });
}
...
function fn_editarMantenimiento() {
    ...
    var url = baseUrl + "Mantenimiento/RegistrarPlazo";
    var item = {
        ID_PLAZO_ETAPA_ESTADO: $("#userMantenimiento").data("value"),
        ETAPA: $("#txt-etapa").val(),
...
function fn_validarCampo() {
    ...
    if ($("#txa-descripcion").val().trim() === "") {
        arr.push("Debe ingresar la descripción");
    }
    if ($("#txt-plazo").val().trim() === "") {
        arr.push("Debe ingresar el plazo");
    }
...
function fn_eliminarMantenimiento() {
    var url = baseUrl + "Mantenimiento/EliminarPlazo";
    var item = {
        ID_NAMA: $("#userMantenimiento").data("value")
    };
    var mensaje = "";
    var respuesta = MRV.Ajax(url, item, false);
    if (respuesta.success) {
...
function exportarMantenimiento() {
    ...
    var url = baseUrl + 'Mantenimiento/ExportarMantenimientoPlazo';
