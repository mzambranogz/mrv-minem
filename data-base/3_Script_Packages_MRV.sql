...
-----------------------------------------------------------------------------------------------------------
    ...
    --------------------------------------------------------------------------
    -- FUNCION FN_Iniciativa_Gen_Dist abarca toda instalaciÛn de generaciÛn 
    -- elÈctrica que inyecte energÌa elÈctrica a la red elÈctrica estÈ conectada 
    -- directamente a la Red de DistribuciÛn. Las tecnologÌas consideradas para la 
    -- generaciÛn distribuida corresponden a fuentes renovables como la generaciÛn 
    -- hidroelÈctrica (pequeÒa escala), eÛlica, fotovoltaica, y biomasa, por lo 
    -- que las reducciones de emisiones se producir·n debido al desplazamiento de 
    -- la generaciÛn elÈctrica con combustibles fÛsiles.
                    
    -- p_elect	: Electricidad generada por centrales tipo solar, eÛlica y mini hidro
    -- p_anno	: AÒo de Inicio de Implementacion de la Iniciativa
    -- Ejemplo :
    ...
    --------------------------------------------------------------------------
    -- FUNCION FN_Iniciativa_Gen_Dist Aumentar la participaciÛn de los Recursos 
    -- EnergÈticos de fuentes Renovables (RER) en la matriz energÈtica nacional 
    -- en un 6.8% en el aÒo 2030, reduciendo la proporciÛn de la energÌa producida 
    -- en base a la quema de combustibles fÛsiles, lo cual generar· la reducciÛn 
    -- de emisiones de gases de efecto invernadero (GEI). Asimismo, la medida 
    -- incluye la implementaciÛn de las centrales hidroelÈctricas con una capacidad 
   ...
   PROCEDURE USP_SEL_LISTA_TIPO_FUENTE(
    ...
    BEGIN
        ...
        PKG_MRV_NOTIFICACION.USP_INS_NOTIFICACION(pIdIniciativa         => pID_INICIATIVA,
                                                  ...
                                                  pDescripcion          => 'El usuario de la entidad ' || TRIM(vEntidad) || 'ha registrado el/los detalle(s) de la Iniciativa (' || vIniciativa || '), en espera de su revisiÛn',
    ...
    BEGIN
        ...
        IF vIdUsuario > 0 THEN      
            ...
            PKG_MRV_NOTIFICACION.USP_INS_NOTIFICACION(pIdIniciativa         => pID_INICIATIVA,
                                                  ...
                                                  pIdUsuario            => vIdUsuario, 
                                                  pIdUsuarioRemitente   => pID_USUARIO,
                                                  pDescripcion          => 'La iniciativa y el detalle de indicadores de la entidad ' || TRIM(vEntidad) || ' fueron revisadas y aprobadas, y estan a la espera de su revisiÛn',
                                                  pIdEstadoNotificacion => 0);
        END IF;
    ...
    BEGIN       
        ...
        VALUES (vIdDetalle, pID_INICIATIVA, pID_USUARIO,3,5, SYSDATE);
            ...
            PKG_MRV_NOTIFICACION.USP_INS_NOTIFICACION(pIdIniciativa         =>  pID_INICIATIVA,
                                                      ...
                                                      pDescripcion          =>  'La entidad ' || TRIM(vEntidad) || ' ha corregido el detalle de indicadores correctamente y requiere su revisiÛn',
                                                      pIdEstadoNotificacion =>  0); 
    ...
    END USP_SEL_BUS_AVAN_PUB;
      ...
      BEGIN
        ...
        SELECT DISTINCT
            ...
            INNER JOIN T_GEND_INICIATIVA_GEI IG ON INI.ID_INICIATIVA = IG.ID_INICIATIVA
            INNER JOIN T_GEND_INICIATIVA_ENERG IE ON INI.ID_INICIATIVA = IE.ID_INICIATIVA
        WHERE   (LOWER(TRANSLATE(INI.NOMBRE_INICIATIVA,'¡…Õ”⁄·ÈÌÛ˙','AEIOUaeiou')) like '%'|| LOWER(TRANSLATE(pBuscar,'¡…Õ”⁄·ÈÌÛ˙','AEIOUaeiou')) ||'%' 
                OR LOWER(TRANSLATE(MD.NOMBRE_MEDMIT,'¡…Õ”⁄·ÈÌÛ˙','AEIOUaeiou')) like'%'|| LOWER(TRANSLATE(pBuscar,'¡…Õ”⁄·ÈÌÛ˙','AEIOUaeiou')) ||'%' OR 
                LOWER(TRANSLATE(INST.NOMBRE_INSTITUCION,'¡…Õ”⁄·ÈÌÛ˙','AEIOUaeiou')) like '%'||LOWER(TRANSLATE(pBuscar,'¡…Õ”⁄·ÈÌÛ˙','AEIOUaeiou'))||'%' ) AND
                INI.PRIVACIDAD_INICIATIVA = '1'
                ORDER BY INI.ID_INICIATIVA DESC;
