...
--////////////////////////////////////////// DINAMICO FINAL
  ...
  PROCEDURE USP_SEL_FILTRAR_PARAMETRO(
         PI_PARAMETRO IN NUMBER,
         PI_DETALLE IN NUMBER,
	 PI_ID_ENFOQUE IN NUMBER,
         PO_RF OUT SYS_REFCURSOR
  );

   PROCEDURE USP_PRC_AIV(
    PI_ID_INICIATIVA NUMBER,
    PI_ID_INDICADOR NUMBER,
    PI_ID_ESTADO NUMBER,
    PI_AUDITADO NUMBER,
    PI_IMPLEMENTADO NUMBER,
    PI_VERIFICADO NUMBER,
    PI_ENERGIA_AUDITADO NUMBER,
    PI_ENERGIA_IMPLEMENTADO NUMBER,
    PI_ENERGIA_VERIFICADO NUMBER,
    PO_ROWAFFECTED OUT NUMBER
  );
  
  PROCEDURE USP_SEL_AIV(
      PI_ID_INICIATIVA NUMBER,
      PI_ID_INDICADOR NUMBER,
      PO_REF OUT SYS_REFCURSOR
  );
...
PROCEDURE USP_SEL_INSTITUCION_ACCION(
  ...
  PROCEDURE USP_SEL_ACCION_MEDIDA(
        PI_ID_USUARIO IN NUMBER,
        PI_ID_MEDMIT IN NUMBER,
  ...
  PROCEDURE USP_PRC_INICIATIVA_AUDITORIA(
        PI_ID_INICIATIVA  IN NUMBER,
        PI_SECTOR_INST IN VARCHAR2,
        PI_INSTITUCION_AUDITADA IN VARCHAR2,
        PI_TIPO_AUDITORIA       IN VARCHAR2,
		PI_DESCRIPCION_TIPO_AUDITORIA IN VARCHAR2,										  
    ...
    PROCEDURE USP_PRC_MONTO_INVERSION (
        PI_ID_INICIATIVA IN NUMBER,
        PI_ANIO IN NUMBER,
        PI_MONEDA IN NUMBER,
        PI_INVERSION IN NUMBER,
        PI_USUARIO_REGISTRO IN NUMBER
    );
    
    PROCEDURE USP_SEL_MONTO_INVERSION(
        PI_ID_INICIATIVA IN NUMBER,
        PO OUT SYS_REFCURSOR
    );
...
PROCEDURE USP_SEL_LISTA_MEDMIT(
  ...
  PROCEDURE USP_SEL_EXCEL_MED_ENF(
   ...
   PROCEDURE USP_SEL_BUSCAR_TIPO_INICIATIVA(
    ...
    PROCEDURE USP_PRC_FILTRO_ENFOQUE_PARAM(
        PI_ID_ENFOQUE     IN NUMBER,
        PO_REF  OUT SYS_REFCURSOR
    );
    
    PROCEDURE USP_PRC_FILTRO_PARAM(
        PI_ID_ENFOQUE     IN NUMBER,
        PI_ID_PARAMETRO     IN NUMBER,
        PO_REF  OUT SYS_REFCURSOR
    );
    
    PROCEDURE USP_SEL_PARAMETRO_RELACION(
        PI_ID_PARAMETRO     IN NUMBER,
        PI_PARAMFILTRO     IN NUMBER,
        PI_ID_ENFOQUE     IN NUMBER,
        PO_REF  OUT SYS_REFCURSOR
    );
    
    PROCEDURE USP_DEL_PARAMETRO_RELACION(
        PI_ID_ENFOQUE     IN NUMBER,
        PI_ID_PARAMETRO     IN NUMBER,
        PI_ID_PARAMETROS     IN NUMBER
    );
    
    PROCEDURE USP_PRC_PARAMETRO_RELACION(
        PI_ID_ENFOQUE     IN NUMBER,
        PI_ID_PARAMETRO     IN NUMBER,
        PI_ID_DETALLE     IN NUMBER,
        PI_PARAMETROS     IN NUMBER,
        PI_DETALLES     IN NUMBER
    );
...
CREATE OR REPLACE  PACKAGE MRVMM."PKG_MRV_REPORTES" AS
  ...
  PROCEDURE USP_SEL_BUS_AVAN_PUB(
    ...
    PROCEDURE USP_SEL_RPT_AUDITORIA(
        PO OUT SYS_REFCURSOR
    );
  ...
  BEGIN
        ...
        SELECT  E.NOMBRE_MEDMIT,
                ...
                (SELECT COUNT(*)FROM (
                  ...
                  LEFT JOIN T_MAE_SECTOR_INST SEC ON INS.ID_SECTOR_INSTITUCION = SEC.ID_SECTOR_INST
                  WHERE IN1.ID_MEDMIT = E.ID_MEDMIT AND ID_SECTOR_INST = 1 AND NVL(IN1.FLG_ESTADO,'1') = '1' AND IN1.ASOCIADO_ACCION = 0
                ) DUAL) CANT_PUBLICO,
                (SELECT COUNT(*)FROM (
                  ...
                  LEFT JOIN T_GENM_INSTITUCION INS ON U.ID_INSTITUCION = INS.ID_INSTITUCION
                  LEFT JOIN T_MAE_SECTOR_INST SEC ON INS.ID_SECTOR_INSTITUCION = SEC.ID_SECTOR_INST
                  WHERE INN.ID_MEDMIT = E.ID_MEDMIT AND ID_SECTOR_INST = 2 AND NVL(INN.FLG_ESTADO,'1') = '1' AND INN.ASOCIADO_ACCION = 0
                ) DUAL) CANT_PRIVADO,
                (SELECT COUNT(*)FROM (
                  SELECT DISTINCT INN.ID_INICIATIVA
                  FROM T_GENM_INICIATIVA INN
                  ...
                  LEFT JOIN T_GENM_INSTITUCION INS ON U.ID_INSTITUCION = INS.ID_INSTITUCION
                  LEFT JOIN T_MAE_SECTOR_INST SEC ON INS.ID_SECTOR_INSTITUCION = SEC.ID_SECTOR_INST
                  WHERE INN.ID_MEDMIT = E.ID_MEDMIT AND ID_SECTOR_INST = 3 AND NVL(INN.FLG_ESTADO,'1') = '1' AND INN.ASOCIADO_ACCION = 0
                ) DUAL) CANT_NATURAL
        FROM    T_MAE_MEDMIT E
 ...
 PROCEDURE SP_SEL_INICIATIVAS_RPT(
  ...
  BEGIN
    ...
    BEGIN
        ...
        LEFT JOIN T_GENM_INSTITUCION INS ON U.ID_INSTITUCION = INS.ID_INSTITUCION
        LEFT JOIN T_MAE_SECTOR_INST SEC ON INS.ID_SECTOR_INSTITUCION = SEC.ID_SECTOR_INST
        WHERE I.ID_MEDMIT = pID_MEDMIT AND I.ASOCIADO_ACCION = 0 AND NVL(I.FLG_ESTADO,'1') = '1'
        GROUP BY NVL(SEC.DESCRIPCION,'SECTOR PUBLICO'), NVL(SEC.ID_SECTOR_INST,1)
        ORDER BY NVL(SEC.ID_SECTOR_INST,1) ASC;
   ...
   PROCEDURE USP_PRC_MOSTRAR_ACUMULADOR(
    ...
    BEGIN
        ...
        SELECT
        MD.NOMBRE_MEDMIT,
        NVL(INIAU.INSTITUCION_AUDITADA, '-') INSTITUCION,
        INI.NOMBRE_INICIATIVA,
        INI.ID_INICIATIVA,
        NVL(INIAU.FECHA_AUDITORIA,'-') FECHA_AUDITADA,
        --NVL((SELECT SUM(TO_NUMBER(NVL(VALOR,0.0),'9999999999990.0000000000000000')) FROM T_MAEM_INDICADOR_DATA WHERE ID_MEDMIT = 4 AND ID_INICIATIVA = INI.ID_INICIATIVA AND ID_PARAMETRO = 11),0) EMISION_AUDITADA,
        NVL((SELECT SUM(AUDITADO) FROM T_GEND_INDICADOR_AIF WHERE ID_INICIATIVA = INI.ID_INICIATIVA
        AND ID_INDICADOR IN (SELECT DISTINCT ID_INDICADOR FROM T_MAEM_INDICADOR_DATA WHERE ID_INICIATIVA = INI.ID_INICIATIVA AND FLAG_ESTADO = '1')),0) EMISION_AUDITADA,
        
        NVL((SELECT MIN(VALOR) FROM T_MAEM_INDICADOR_DATA 
        WHERE ID_MEDMIT = 4 AND ID_INICIATIVA = INI.ID_INICIATIVA AND ID_PARAMETRO = 92 AND ID_INDICADOR IN (SELECT ID_INDICADOR FROM T_MAEM_INDICADOR_DATA WHERE ID_MEDMIT = 4 AND ID_INICIATIVA = INI.ID_INICIATIVA AND ID_PARAMETRO = 91 AND VALOR >= 2)),'-') FECHA_IMPLEMENTADA,
        --NVL((SELECT SUM(TO_NUMBER(NVL(VALOR,0.0),'9999999999990.0000000000000000')) FROM T_MAEM_INDICADOR_DATA 
        --WHERE ID_MEDMIT = 4 AND ID_INICIATIVA = INI.ID_INICIATIVA AND ID_PARAMETRO = 11 AND ID_INDICADOR IN (SELECT ID_INDICADOR FROM T_MAEM_INDICADOR_DATA WHERE ID_MEDMIT = 4 AND ID_INICIATIVA = INI.ID_INICIATIVA AND ID_PARAMETRO = 91 AND VALOR > 1)),0) EMISION_IMPLEMENTADA,
        NVL((SELECT SUM(IMPLEMENTADO) FROM T_GEND_INDICADOR_AIF WHERE ID_INICIATIVA = INI.ID_INICIATIVA
        AND ID_INDICADOR IN (SELECT DISTINCT ID_INDICADOR FROM T_MAEM_INDICADOR_DATA WHERE ID_INICIATIVA = INI.ID_INICIATIVA AND FLAG_ESTADO = '1')),0) EMISION_IMPLEMENTADA,
        
        NVL((SELECT MIN(VALOR) FROM T_MAEM_INDICADOR_DATA 
        WHERE ID_MEDMIT = 4 AND ID_INICIATIVA = INI.ID_INICIATIVA AND ID_PARAMETRO = 94 AND ID_INDICADOR IN (SELECT ID_INDICADOR FROM T_MAEM_INDICADOR_DATA WHERE ID_MEDMIT = 4 AND ID_INICIATIVA = INI.ID_INICIATIVA AND ID_PARAMETRO = 91 AND VALOR = 3)),'-') FECHA_VERIFICADA,
        --NVL((SELECT SUM(TO_NUMBER(NVL(VALOR,0.0),'9999999999990.0000000000000000')) FROM T_MAEM_INDICADOR_DATA 
        --WHERE ID_MEDMIT = 4 AND ID_INICIATIVA = INI.ID_INICIATIVA AND ID_PARAMETRO = 11 AND ID_INDICADOR IN (SELECT ID_INDICADOR FROM T_MAEM_INDICADOR_DATA WHERE ID_MEDMIT = 4 AND ID_INICIATIVA = INI.ID_INICIATIVA AND ID_PARAMETRO = 91 AND VALOR > 2)),0) EMISION_VERIFICADA
        NVL((SELECT SUM(VERIFICADO) FROM T_GEND_INDICADOR_AIF WHERE ID_INICIATIVA = INI.ID_INICIATIVA
        AND ID_INDICADOR IN (SELECT DISTINCT ID_INDICADOR FROM T_MAEM_INDICADOR_DATA WHERE ID_INICIATIVA = INI.ID_INICIATIVA AND FLAG_ESTADO = '1')),0) EMISION_VERIFICADA
        
        FROM T_GENM_INICIATIVA INI
        INNER JOIN T_MAE_MEDMIT MD ON INI.ID_MEDMIT = MD.ID_MEDMIT
        LEFT JOIN T_GEND_INICIATIVA_AUDITORIA INIAU ON INI.ID_INICIATIVA = INIAU.ID_INICIATIVA
        WHERE INI.ID_MEDMIT = 4 AND NVL(INI.FLG_ESTADO,'1') = '1';

    END USP_SEL_RPT_AUDITORIA;
...
CREATE OR REPLACE PACKAGE BODY MRVMM."PKG_MRV_ADMIN_SISTEMA" AS
  ...
  PROCEDURE USP_SEL_REDUCIDOS_MEDIDA(
    ...
    BEGIN
        ...
        WHERE INS.RUC_INSTITUCION = '''|| PI_RUC ||''' AND INI.ID_INICIATIVA NOT IN ('|| PI_ID_INICIATIVAS ||') AND NVL(INI.FLG_ESTADO,''1'') = ''1'' AND INI.USUARIO_REGISTRO = USU.ID_USUARIO
 ...
 PROCEDURE USP_PRC_INICIATIVA_AUDITORIA(
    ...
    BEGIN
        ...
        IF vContador > 0 THEN
            ...
            SET     
                    SECTOR_INST = PI_SECTOR_INST,
                    INSTITUCION_AUDITADA = PI_INSTITUCION_AUDITADA,
                    TIPO_AUDITORIA = PI_TIPO_AUDITORIA,
					DESCRIPCION_TIPO_AUDITORIA = PI_DESCRIPCION_TIPO_AUDITORIA,														   
                    AUDITOR = PI_AUDITOR,
        ...
        ELSE
            ...
            INSERT INTO T_GEND_INICIATIVA_AUDITORIA (ID_AUDITORIA, ID_INICIATIVA, SECTOR_INST, INSTITUCION_AUDITADA, TIPO_AUDITORIA, DESCRIPCION_TIPO_AUDITORIA, AUDITOR, NOMBRE_INSTITUCION, FECHA_AUDITORIA)
            VALUES (vContador, PI_ID_INICIATIVA, PI_SECTOR_INST, PI_INSTITUCION_AUDITADA, PI_TIPO_AUDITORIA, PI_DESCRIPCION_TIPO_AUDITORIA, PI_AUDITOR, PI_NOMBRE_INSTITUCION, PI_FECHA_AUDITORIA);
        END IF;
  ...
  PROCEDURE USP_UPD_INICIATIVA_DELETE(
	...
	UPDATE T_GENM_INICIATIVA SET FLG_ESTADO = '0' WHERE ASOCIADO_ACCION = PI_ID_INICIATIVA;
        UPDATE T_GENM_ACUMULADO SET BAU = 0, INI = 0, REDUCIDO = 0 WHERE ID_INICIATIVA IN (SELECT ID_INICIATIVA FROM T_GENM_INICIATIVA WHERE ASOCIADO_ACCION = PI_ID_INICIATIVA);
        UPDATE T_MAEM_INDICADOR_DATA SET FLAG_ESTADO = '0' WHERE ID_INICIATIVA IN (SELECT ID_INICIATIVA FROM T_GENM_INICIATIVA WHERE ASOCIADO_ACCION = PI_ID_INICIATIVA);
  ...
  PROCEDURE USP_SEL_MONEDA(
    ...
    BEGIN
        ...
        IF pID_ESTADO = 0 THEN
            INSERT INTO T_GENM_INICIATIVA(ID_INICIATIVA, ID_MEDMIT, ID_USUARIO, NOMBRE_INICIATIVA, DESC_INICIATIVA, ID_ESTADO, PRIVACIDAD_INICIATIVA, PRIVACIDAD_INVERSION, INVERSION_INICIATIVA,ID_MONEDA,FECHA_IMPLE_INICIATIVA, FECHA_FIN_INICIATIVA,ID_ETAPA, FECHA_CREA_INICIATIVA, FECHA_REGISTRO, GEI_TOTAL, ID_TIPO_INICIATIVA, NDC, USUARIO_REGISTRO, ASOCIADO_ACCION )
            VALUES (vIdIniciativa,pID_MEDMIT,pID_USUARIO,pNOMBRE_INICIATIVA,pDESC_INICIATIVA,pID_ESTADO,pPRIVACIDAD_INICIATIVA, pPRIVACIDAD_INVERSION,pINVERSION_INICIATIVA,pID_MONEDA,pFECHA_IMPLE_INICIATIVA, pFECHA_FIN_INICIATIVA, 1, SYSDATE, SYSDATE, 0, pID_TIPO_INICIATIVA, pNDC, pID_USUARIO, pASOCIADO_ACCION);
        ELSE
            INSERT INTO T_GENM_INICIATIVA(ID_INICIATIVA, ID_MEDMIT, ID_USUARIO, NOMBRE_INICIATIVA, DESC_INICIATIVA, ID_ESTADO, PRIVACIDAD_INICIATIVA, PRIVACIDAD_INVERSION, INVERSION_INICIATIVA,ID_MONEDA,FECHA_IMPLE_INICIATIVA, FECHA_FIN_INICIATIVA,ID_ETAPA, FECHA_CREA_INICIATIVA, FECHA_REGISTRO, GEI_TOTAL, ID_TIPO_INICIATIVA, NDC, USUARIO_REGISTRO, ASOCIADO_ACCION )
            VALUES (vIdIniciativa,pID_MEDMIT,pID_USUARIO,pNOMBRE_INICIATIVA,pDESC_INICIATIVA,pID_ESTADO,pPRIVACIDAD_INICIATIVA, pPRIVACIDAD_INVERSION,pINVERSION_INICIATIVA,pID_MONEDA,pFECHA_IMPLE_INICIATIVA, pFECHA_FIN_INICIATIVA, 1, SYSDATE, SYSDATE, 0, pID_TIPO_INICIATIVA, pNDC, pID_USUARIO, pASOCIADO_ACCION);
        END IF;
        ...
        --======================MONEDA
        INSERT INTO T_GEND_INICIATIVA_INVERSION (ID_INICIATIVA, ANIO, MONEDA, INVERSION, USUARIO_REGISTRO, FECHA_REGISTRO)
        VALUES (vIdIniciativa, EXTRACT(YEAR FROM pFECHA_IMPLE_INICIATIVA), pID_MONEDA, pINVERSION_INICIATIVA, pID_USUARIO, SYSDATE);
        --======================
        ...
        IF pID_ESTADO = 1 THEN
                ...
                PKG_MRV_NOTIFICACION.USP_INS_NOTIFICACION(pIdIniciativa         => vIdIniciativa,
                                                          ...
                                                          pDescripcion          => 'La acción de la entidad ' || TRIM(vEntidad) || ' ha sido registrado correctamente y requiere su revisión',
   ...
   PROCEDURE USP_UPD_INICIATIVA_MITIGACION(
    ...
    BEGIN
        ...
        IF (pID_ESTADO = 0 OR pID_ESTADO = 6) THEN
               ...
               SET
                   ...
                   NDC = pNDC,
                   USUARIO_MODIFICA       = pID_USUARIO,
                   ASOCIADO_ACCION       = pASOCIADO_ACCION
               WHERE ID_INICIATIVA        = pID_INICIATIVA ;
        ...
        SELECT COUNT(*) INTO vId FROM T_GEND_INICIATIVA_INVERSION WHERE ID_INICIATIVA = pID_INICIATIVA AND ANIO = EXTRACT(YEAR FROM pFECHA_IMPLE_INICIATIVA);

        IF vId = 0 THEN
            UPDATE T_GEND_INICIATIVA_INVERSION SET INVERSION = 0 WHERE ID_INICIATIVA = pID_INICIATIVA;
            INSERT INTO T_GEND_INICIATIVA_INVERSION (ID_INICIATIVA, ANIO, MONEDA, INVERSION, USUARIO_REGISTRO, FECHA_REGISTRO)
            VALUES (pID_INICIATIVA, EXTRACT(YEAR FROM pFECHA_IMPLE_INICIATIVA), pID_MONEDA, pINVERSION_INICIATIVA, pID_USUARIO, SYSDATE);
        ELSE
            UPDATE T_GEND_INICIATIVA_INVERSION SET
            MONEDA = pID_MONEDA,
            INVERSION = pINVERSION_INICIATIVA,
            USUARIO_MODIFICA = pID_USUARIO,
            FECHA_MODIFICA = SYSDATE
            WHERE ID_INICIATIVA = pID_INICIATIVA AND ANIO = EXTRACT(YEAR FROM pFECHA_IMPLE_INICIATIVA);
        END IF;
        --==================================================
    ...
    BEGIN
        ...
        SELECT INI.ID_INICIATIVA,
	       ...
	       IA.SECTOR_INST, --ADD 03-11-20
               IA.INSTITUCION_AUDITADA, --ADD 03-11-20
               IA.TIPO_AUDITORIA, --ADD 26-10-20
			   IA.DESCRIPCION_TIPO_AUDITORIA, --04-11-20								 
    ...
    BEGIN
        ...
        SELECT  ID_MEDMIT,
                NOMBRE_MEDMIT
        FROM    T_MAE_MEDMIT
        WHERE   NVL(ASOCIADO,0) IN (0,pID_USUARIO);
...
--(INI.ID_ESTADO = 3 AND INI.ID_ETAPA IN (2,4))
    ...
    BEGIN
        ...
        IF vId = 0 THEN
            INSERT INTO T_GEND_INICIATIVA_INVERSION (ID_INICIATIVA, ANIO, MONEDA, INVERSION, USUARIO_REGISTRO, FECHA_REGISTRO)
            VALUES (PI_ID_INICIATIVA, PI_ANIO, PI_MONEDA, PI_INVERSION, PI_USUARIO_REGISTRO, SYSDATE);
        ELSE
            UPDATE T_GEND_INICIATIVA_INVERSION SET
            MONEDA = PI_MONEDA,
            INVERSION = PI_INVERSION,
            USUARIO_MODIFICA = PI_USUARIO_REGISTRO,
            FECHA_MODIFICA = SYSDATE
        ...
        ELSE            
            ...
            IF vMonedaAsociado = 0 THEN
                ...
                UPDATE T_GENM_INICIATIVA SET ID_MONEDA = PI_MONEDA WHERE ID_INICIATIVA IN (SELECT ID_INICIATIVA 
                                                                                               FROM T_GENM_INICIATIVA
                                                                                               WHERE ASOCIADO_ACCION = PI_ID_INICIATIVA OR ID_INICIATIVA = PI_ID_INICIATIVA);
                UPDATE T_GEND_INICIATIVA_INVERSION SET MONEDA = PI_MONEDA WHERE ID_INICIATIVA IN (SELECT ID_INICIATIVA 
                                                                                               FROM T_GENM_INICIATIVA
                                                                                               WHERE ASOCIADO_ACCION = PI_ID_INICIATIVA OR ID_INICIATIVA = PI_ID_INICIATIVA);
            END IF;
    ...
    BEGIN
        OPEN PO FOR
        SELECT * FROM T_GEND_INICIATIVA_INVERSION
        WHERE ID_INICIATIVA = PI_ID_INICIATIVA;
    END USP_SEL_MONTO_INVERSION;
...
--------------------
  ...
  BEGIN
    ...
    SELECT ID_ESTADO_USUARIO INTO vCount FROM T_GENM_USUARIO WHERE ID_USUARIO = pID_USUARIO;
    IF vCount = 2 THEN
      --ACTUALIZACION
        UPDATE T_MAE_USUARIO_MEDMIT U
        SET U.FLG_ESTADO     = '0'
...
PROCEDURE USP_SEL_EXCEL_INSTITUCION(
 ...
 --///////////////////////// DINAMICO INICIO
  ...
  BEGIN
        OPEN pRefcursor FOR
        SELECT  FD.ID_DETALLE, FD.ID_PARAMETRO, FD.VALOR, FD.FACTOR, FD.UNIDAD
  ...
  BEGIN
        ...
        IF vContador = 0 THEN
            ...
            INSERT INTO T_MAEM_FACTOR_DATA (ID_FACTOR, ID_DETALLE, ID_PARAMETRO, VALOR, FACTOR, UNIDAD, FLAG_ESTADO)
            VALUES (pID_FACTOR, vIdDetalle, pID_PARAMETRO, pVALOR, pFACTOR, pUNIDAD,'1');
        ELSE
  ...
  PROCEDURE USP_INS_TIPO_INICIATIVA(
   ...
   PROCEDURE USP_GET_PARAMETRO(
    ...
    BEGIN
      OPEN PO_REF FOR
      SELECT
      MI.ID_PARAMETRO, MP.NOMBRE_PARAMETRO
      FROM T_MAEM_INDICADOR MI
      INNER JOIN T_MAEM_MRV_PARAMETRO MP ON MI.ID_PARAMETRO = MP.ID_PARAMETRO
      WHERE MI.FLAG_ESTADO = '1' AND MP.ID_TIPO_CONTROL = '1' AND MI.ID_ENFOQUE = PI_ID_ENFOQUE;

    END USP_PRC_FILTRO_ENFOQUE_PARAM;
    ...
    BEGIN
      ...
      FROM T_MAEM_INDICADOR MI
      INNER JOIN T_MAEM_MRV_PARAMETRO MP ON MI.ID_PARAMETRO = MP.ID_PARAMETRO
      WHERE MI.FLAG_ESTADO = '1' AND MP.ID_TIPO_CONTROL = '1' AND MI.ID_ENFOQUE = PI_ID_ENFOQUE AND NOT MI.ID_PARAMETRO = PI_ID_PARAMETRO;

    END USP_PRC_FILTRO_PARAM;
    ...
    BEGIN
      OPEN PO_REF FOR
      SELECT      *
      FROM T_MAED_PARAMETRO_RELACION
      WHERE FLAG_ESTADO = '1' AND ID_ENFOQUE = PI_ID_ENFOQUE AND ID_PARAMETRO = PI_ID_PARAMETRO AND PARAMETROS = PI_PARAMFILTRO;

    END USP_SEL_PARAMETRO_RELACION;
    ...
    BEGIN
      UPDATE  T_MAED_PARAMETRO_RELACION SET FLAG_ESTADO = '0' 
      WHERE ID_ENFOQUE = PI_ID_ENFOQUE AND ID_PARAMETRO = PI_ID_PARAMETRO AND PARAMETROS = PI_ID_PARAMETROS;

    END USP_DEL_PARAMETRO_RELACION;
    ...
    PROCEDURE USP_PRC_PARAMETRO_RELACION(
        ...
        VCONT NUMBER;
    BEGIN
      SELECT COUNT(*) INTO VCONT FROM T_MAED_PARAMETRO_RELACION WHERE ID_ENFOQUE = PI_ID_ENFOQUE AND ID_PARAMETRO = PI_ID_PARAMETRO AND PARAMETROS = PI_PARAMETROS AND ID_DETALLE = PI_ID_DETALLE;
      IF VCONT = 0 THEN
          INSERT INTO T_MAED_PARAMETRO_RELACION (ID_ENFOQUE, ID_PARAMETRO, ID_DETALLE, PARAMETROS, DETALLES, FLAG_ESTADO)
          VALUES (PI_ID_ENFOQUE, PI_ID_PARAMETRO, PI_ID_DETALLE, PI_PARAMETROS, PI_DETALLES, '1');
      ELSE
          UPDATE  T_MAED_PARAMETRO_RELACION SET 
          DETALLES = PI_DETALLES,
          FLAG_ESTADO = '1' 
          WHERE ID_ENFOQUE = PI_ID_ENFOQUE AND ID_PARAMETRO = PI_ID_PARAMETRO AND PARAMETROS = PI_PARAMETROS AND ID_DETALLE = PI_ID_DETALLE;
      END IF;
      

    END USP_PRC_PARAMETRO_RELACION;
...
CREATE OR REPLACE  PACKAGE BODY MRVMM."PKG_MRV_PARAMETROS" is
    ...
    BEGIN
        IF PI_ID_DETALLE = 0 THEN
            ...
            INSERT INTO T_MAEM_MRV_FACTOR_PARAMETRO (ID_FACTOR, ID_DETALLE, ID_TIPO_CONTROL, ID_TIPO_DATO, NOMBRE_DETALLE, ORDEN, FLAG_ESTADO)
            VALUES (PI_ID_FACTOR, 101, 2, 3, 'Unidad', 101, '1');
        ELSE
...
--PKG_MRV_DETALLE_INDICADORES.USP_UPD_OBSERVACION_DETALLE
  ...
  BEGIN
        OPEN pRefcursor FOR
        SELECT  MI.ID_ENFOQUE, MP.NOMBRE_PARAMETRO, MI.ORDEN, MI.ID_GRUPO_INDICADOR, MGI.COLOR_GRUPO, MI.ID_PARAMETRO,
                MP.ID_TIPO_CONTROL, MP.EDITABLE, MP.ID_TIPO_DATO, MP.VERIFICABLE, V_DECIMAL, MP.VISIBLE 
                , MP.ENERGIA_TOTAL, MP.VALIDAR_IMPLEMENTADO 
				, MP.VALIDAR_FECHA_IMPLEM, MP.VALIDAR_VERIFICACION, MP.VALIDAR_FECHA_VERIFI, MP.FILTRO															 
        FROM    T_MAEM_INDICADOR MI
        LEFT JOIN   T_MAEM_MRV_PARAMETRO MP ON MI.ID_PARAMETRO = MP.ID_PARAMETRO
        LEFT JOIN   T_MAMEM_GRUPO_INDICADOR MGI ON MI.ID_GRUPO_INDICADOR = MGI.ID_GRUPO_INDICADOR
        WHERE MI.ID_MEDMIT = pID_MEDMIT AND MI.ID_ENFOQUE = pID_ENFOQUE
        AND MI.FLAG_ESTADO = '1' 
        ORDER BY MI.ID_ENFOQUE ,MI.ORDEN ASC;
 ...
 PROCEDURE USP_UPD_ELIMINAR_IND_DATA(
    ...
    BEGIN
        ...
        vSql2 := 'UPDATE T_GENM_ACUMULADO SET BAU = 0, INI = 0, REDUCIDO = 0, ENERGIA = 0, ENERGIA_TOTAL = 0 WHERE ID_INICIATIVA ='||pID_INICIATIVA||' AND ID_INDICADOR IN ('||pID_INDICADOR_DELETE||')';
  ...
  BEGIN
		...
		MP.VISIBLE, 
		MP.ENERGIA_TOTAL, MP.VALIDAR_IMPLEMENTADO, 
                MP.VALIDAR_FECHA_IMPLEM, MP.VALIDAR_VERIFICACION, MP.VALIDAR_FECHA_VERIFI, 										   																										 
...
PROCEDURE USP_INS_ACUMULADO_DETALLE(
  ...
  BEGIN
        ...
        SELECT  *
        FROM    T_MAED_PARAMETRO_RELACION
        WHERE   ID_PARAMETRO = PI_PARAMETRO AND ID_DETALLE = PI_DETALLE AND ID_ENFOQUE = PI_ID_ENFOQUE AND FLAG_ESTADO = '1';
  END USP_SEL_FILTRAR_PARAMETRO;
  ...
  PROCEDURE USP_PRC_AIV(
    ...
    vID NUMBER;
  BEGIN
    SELECT COUNT(*) INTO vID FROM T_GEND_INDICADOR_AIF WHERE ID_INICIATIVA = PI_ID_INICIATIVA AND ID_INDICADOR = PI_ID_INDICADOR;
    IF vID = 0 THEN
      INSERT INTO T_GEND_INDICADOR_AIF
      (ID_INICIATIVA, ID_INDICADOR, ID_ESTADO, AUDITADO, IMPLEMENTADO, VERIFICADO, ENERGIA_AUDITADO, ENERGIA_IMPLEMENTADO, ENERGIA_VERIFICADO)
      VALUES
      (PI_ID_INICIATIVA, PI_ID_INDICADOR, PI_ID_ESTADO, PI_AUDITADO, PI_IMPLEMENTADO, PI_VERIFICADO, PI_ENERGIA_AUDITADO, PI_ENERGIA_IMPLEMENTADO, PI_ENERGIA_VERIFICADO);
    ELSE
      UPDATE T_GEND_INDICADOR_AIF SET
      ID_ESTADO = PI_ID_ESTADO,
      AUDITADO = PI_AUDITADO,
      IMPLEMENTADO = PI_IMPLEMENTADO,
      ENERGIA_AUDITADO = PI_ENERGIA_AUDITADO,
      ENERGIA_IMPLEMENTADO = PI_ENERGIA_IMPLEMENTADO,
      ENERGIA_VERIFICADO = PI_ENERGIA_VERIFICADO,
      VERIFICADO = PI_VERIFICADO
      WHERE ID_INICIATIVA = PI_ID_INICIATIVA AND ID_INDICADOR = PI_ID_INDICADOR;
    END IF;
