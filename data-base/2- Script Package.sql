prompt PL/SQL Developer Export User Objects for user MRVMM@MEM9DESA_12
prompt Created by CORPORACION-ZUNIGA on domingo, 31 de Mayo de 2020
set define off
spool 5-Package.log
...
CREATE OR REPLACE PACKAGE BODY MRVMM."PKG_MRV_REPORTES" AS
  ...
  BEGIN
        ...
        SELECT  E.NOMBRE_MEDMIT,
                ...
                (--SELECT COUNT(*)FROM (
                  SELECT COUNT(DISTINCT IN1.ID_INICIATIVA) 
                  ...
                  LEFT JOIN T_MAE_SECTOR_INST SEC ON INS.ID_SECTOR_INSTITUCION = SEC.ID_SECTOR_INST
                  WHERE IN1.ID_MEDMIT = E.ID_MEDMIT AND ID_SECTOR_INST = 1
                /*) DUAL*/) CANT_PUBLICO,
                (--SELECT COUNT(*)FROM (
                  SELECT COUNT(DISTINCT INN.ID_INICIATIVA)
                  FROM T_GENM_INICIATIVA INN
                  INNER JOIN T_GENM_USUARIO U ON INN.ID_USUARIO = U.ID_USUARIO
                  LEFT JOIN T_GENM_INSTITUCION INS ON U.ID_INSTITUCION = INS.ID_INSTITUCION
                  LEFT JOIN T_MAE_SECTOR_INST SEC ON INS.ID_SECTOR_INSTITUCION = SEC.ID_SECTOR_INST
                  WHERE INN.ID_MEDMIT = E.ID_MEDMIT AND ID_SECTOR_INST = 2
                /*) DUAL*/) CANT_PRIVADO
        FROM    T_MAE_MEDMIT E;
